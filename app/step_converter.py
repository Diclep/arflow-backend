...
from OCC.Core.TopologyUtils import TopologyExplorer
...
def convert_step_to_glb(step_path: str, output_path: str, mesh_quality: float = 0.1) -> dict:
    ...
    for i in range(1, labels.Length() + 1):
        ...
        # Nome del componente (se presente nel file STEP)
        name_attr = label.GetLabelName() if hasattr(label, 'GetLabelName') else f"component_{i}"
        comp_name = name_attr if name_attr else f"component_{i}"
