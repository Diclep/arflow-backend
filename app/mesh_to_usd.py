...
Restituisce anche i metadata (triangle_count, bounding_box, hierarchy)
da salvare in `models.triangle_count` e `models.auto_metadata_json`.
"""
import trimesh
from pxr import Usd, UsdGeom, Kind


def convert_mesh_to_usd(input_path: str, output_usd_path: str, asset_name: str) -> dict:
    """
    ...
    Ritorna un dict: {"triangle_count": int, "bounding_box": {...}, "hierarchy": [...]}
    """
    ...
    if isinstance(loaded, trimesh.Scene):
        ...
        for idx, (node_name, mesh) in enumerate(loaded.geometry.items()):
            ...
            _write_mesh_prim(stage, root_path, prim_name, mesh)
        ...
        _write_mesh_prim(stage, root_path, prim_name, loaded)
